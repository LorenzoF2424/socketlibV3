...
class Socket {
   public:
      ...
      void listenTCP(int MAX_ACCEPT,int MAX_CLIENTS) {
         this->MAX_CONNECTIONS=MAX_CLIENTS;
         if (listen(this->sockfdTCP, MAX_ACCEPT) < 0) {
            close(this->sockfdTCP);
      ...
      int acceptTCP() {
         ...
         bool giaAvvisato=false;
         while(this->ACTIVE_CLIENTS>=MAX_CONNECTIONS) {
            if (!giaAvvisato) {
               cout<<"Warning: Numero Massimo di Client Raggiunto, rifiutando la connessione..."<<endl;
         ...
         c.sockfd=newsockfd;
         c.address=sock;
         if(this->deleted_clients.size()==0) {
            this->client_list.push_back(c);
            this->ACTIVE_CLIENTS++;
            return this->client_list.size()-1;
         } else {
            int i=this->deleted_clients[0];
            this->client_list[i]=c;
            this->ACTIVE_CLIENTS++;
            this->deleted_clients.erase(this->deleted_clients.begin()+0);
            return i;
         }
         ...
         if((numBytes = recv(this->client_list[i].sockfd,(void*)p,sizeof(T),0))<=0) {
            ...
            if (numBytes==0) {
                ...
                pthread_mutex_lock(&this->mutex);
                    this->deleted_clients.push_back(i);
                    this->ACTIVE_CLIENTS--;
